	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'your creator' | 'rage music':
					curStage = 'void';
				case 'looperman' | 'hobbies' | 'dear friend' | 'farewell':
					curStage = 'plaza';
				default:
					curStage = 'stage';
			}
		...
		switch (curStage)
		{
			...
			case 'plaza':
				if (SONG.song.toLowerCase() == "farewell") {
					var sky:BGSprite = new BGSprite('sky2', -275, -50, 0.9, 0.9);
					sky.setGraphicSize(Std.int(sky.width * 1.1));
					sky.updateHitbox();
					add(sky);
					var bg:BGSprite = new BGSprite('bg2', -275, -50, 0.6, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 1.1));
					bg.updateHitbox();
					add(bg);
					var fence:BGSprite = new BGSprite('fence2', -275, -50, 0.6, 0.9);
					fence.setGraphicSize(Std.int(fence.width * 1.1));
					fence.updateHitbox();
					add(fence);
					var ground:BGSprite = new BGSprite('ground2', -275, -50, 0.9, 0.9);
					ground.setGraphicSize(Std.int(ground.width * 1.1));
					ground.updateHitbox();
					add(ground);
					var tree:BGSprite = new BGSprite('tree2', -275, -100, 0.8, 0.9);
					tree.setGraphicSize(Std.int(tree.width * 1.1));
					tree.updateHitbox();
					add(tree);
					var paper:BGSprite = new BGSprite('paper2', -275, -50, 0.8, 0.9);
					paper.setGraphicSize(Std.int(paper.width * 1.1));
					paper.updateHitbox();
					add(paper);
					upperBoppers = new BGSprite('boppers', -225, 175, 0.9, 0.9, ['crowd bop']);
					upperBoppers.setGraphicSize(Std.int(upperBoppers.width * 1.1));
					upperBoppers.updateHitbox();
					upperBoppers.visible = false;
				}
				else {
					var sky:BGSprite = new BGSprite('sky', -275, -50, 0.9, 0.9);
					sky.setGraphicSize(Std.int(sky.width * 1.1));
					sky.updateHitbox();
					add(sky);
					var bg:BGSprite = new BGSprite('bg', -275, -50, 0.6, 0.9);
					bg.setGraphicSize(Std.int(bg.width * 1.1));
					bg.updateHitbox();
					add(bg);
					var fence:BGSprite = new BGSprite('fence', -275, -50, 0.6, 0.9);
					fence.setGraphicSize(Std.int(fence.width * 1.1));
					fence.updateHitbox();
					add(fence);
					var ground:BGSprite = new BGSprite('ground', -275, -50, 0.9, 0.9);
					ground.setGraphicSize(Std.int(ground.width * 1.1));
					ground.updateHitbox();
					add(ground);
					var tree:BGSprite = new BGSprite('tree', -275, -100, 0.8, 0.9);
					tree.setGraphicSize(Std.int(tree.width * 1.1));
					tree.updateHitbox();
					add(tree);
					var paper:BGSprite = new BGSprite('paper', -275, -50, 0.8, 0.9);
					paper.setGraphicSize(Std.int(paper.width * 1.1));
					paper.updateHitbox();
					add(paper);
					upperBoppers = new BGSprite('boppers', -225, 175, 0.9, 0.9, ['crowd bop']);
					...
					upperBoppers.updateHitbox();
					add(upperBoppers);
					if (SONG.song.toLowerCase() == "looperman") {
						upperBoppers.visible = false;
					}
					if (SONG.song.toLowerCase() == "lovely remix") {
						upperBoppers.visible = false;
						maavo = new BGSprite('MaavoWait', -200, 100, 0.9, 0.9, ['MaavoWait']);
						maavo.setGraphicSize(Std.int(maavo.width * 0.9));
						maavo.antialiasing = true;
						maavo.updateHitbox();
						add(maavo);
					}
				}
			case 'void': //Week 1
				var bg:BGSprite = new BGSprite('void', -600, -200, 0.9, 0.9);
				add(bg);
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'senpai' | 'roses' | 'thorns':
					...
					schoolIntro(doof);
				case 'hobbies' | 'dear friend' | 'farewell' | 'looperman':
					startDialogue(dialogueJson);
			/*	case 'looperman':
					video.playMP4(Paths.video('cutscene'), false, null, false, false);
